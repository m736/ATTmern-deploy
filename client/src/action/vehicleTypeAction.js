...
export const createVehicleTypeAction = (formData) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(
      `http://localhost:4000/api/v1/vehicle_type_api`,
      formData
    );
  ...
  (id, updatedVehicleTypeData) => async (dispatch) => {
    try {
      ...
      const { data } = await axios.put(
        `http://localhost:4000/api/v1/vehicle_type_api/${id}`,
...
export const deleteVehicleTypeAction = (id) => async (dispatch) => {
  try {
    ...
    await axios.delete(`http://localhost:4000/api/v1/vehicle_type_api/${id}`);
